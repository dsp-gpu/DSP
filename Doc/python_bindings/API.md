...
```python
ctx = dsp_core.GPUContext(device_index: int = 0)
```
...
```python
gen = dsp_core.SignalGenerator(ctx: GPUContext)
```
...
```python
fft = dsp_core.FFTProcessor(ctx: GPUContext)
```
...
```python
sg = dsp_core.ScriptGenerator(ctx: GPUContext)
```
